...
\begin{enumerate}
  ...
  \sol {
    $\norm{A\vec{v}}^{2} = (A \vec{v})^{T} (A \vec{v}) = \vec{v}^{T}A^{T} A \vec{v} = \vec{v}^{T} (\lambda \vec{v}) = \lambda \vec{v}^{T} \vec{v} = \lambda \norm{\vec{v}}^{2}.$ \vskip 1pt
    Therefore we see that:
    \begin{equation}
      \lambda = \frac{\norm{A \vec{v}}^{2}}{\norm{\vec{v}}^{2}} \geq 0
    \end{equation}
    $\vec{v}$ is an eigenvector, so it must be nonzero, meaning its norm squared will be greater than 0. \vskip 1pt
    $\norm{A\vec{v}}^{2}$ will also be greater than or equal to zero, but may be zero if $A \vec{v} = 0$ or $\vec{v} \in \text{Nul}(A).$
  }
  ...
  \sol {
  	...
  	Therefore, we can pick $n$ linearly independent eigenvectors of $A^{T} A$ that are all mutually orthogonal, and can normalize each one to have norm $1$ to make them mutually orthonormal.
  }
\end{enumerate}