...
\sol {
    ...
    \begin{align*}
        ...
        \lambda^2 - 2\lambda f_1 - 4f_2 + 4 = 0.
    \end{align*}

    In order for the system to be stable, the eigenvalues must have a negative real component. So, we must be able to factor the characteristic polynomial into the form
    \begin{align*}
        (\lambda + u) (\lambda + v) = 0,
    \end{align*}
    where $u$ and $v$ are both positive.

    We also want the system to have purely real eigenvalues, so it must be in the form
    \begin{align*}
        \lambda^2 + b\lambda + c = 0,
    \end{align*}
    where $b$ and $c$ are both positive.

    So, we can choose any $f_1 < 0$ and any $f_2 < 1$.
}
\end{enumerate}