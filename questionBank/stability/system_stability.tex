...
\begin{enumerate}
  \qitem Show that if the system represented by equation \eqref{eqn:schur} is stable in $z_i$ coordinates, then it must also be stable in $x_i$ coordinates.
  ...
  \qitem Uncouple the system in equation \eqref{eqn:input} into first order scalar differential equations.
  ...
  \qitem When will the differential equation $\ddt{}{t} z_{n}(t) = \lambda_{n} z_{n} + \widetilde{u}_{n}(t)$ be stable?
  ...
  \qitem Using back-substitution, confirm that $\ddt{}{t} \vec{z}(t) = R \vec{z}(t) + \vec{\widetilde{u}}(t)$ is stable only when $\mathfrak{Re}(\lambda_i) < 0$ for all $i.$
  ...
  \sol {
    ...
    We can treat $w_{n-1}(t) = r_{n-1, n} \ z_{n}(t) + \widetilde{u}_{n-1}(t)$ as an input to the differential equation for $z_{n-1},$ and again reapply the definition of BIBO stability to claim that $z_{n-1}$ is stable when $\mathfrak{Re}(\lambda_{n-1}) < 0.$ \vskip 1pt
    Continuing this back-substitution, we will eventually see that all of the $z_i$ are stable only when $\mathfrak{Re}(\lambda_{i}) < 0$ for all $i.$
