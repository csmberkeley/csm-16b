...
\qns{Moore-Penrose Pseudo Inverse}
...
\begin{enumerate}
  ...
  \qitem We will first assume that $A$ is full rank or $\text{Rank}(A) = n$ and try undoing the SVD. What will the $\Sigma$ matrix look like?

  \sol {
    Since $A$ is an $m \times n$ matrix, $\Sigma$ will be also be $m \times n.$
    We assumed that $n > m,$ and that $A$ is full rank, so $\Sigma$ will be a matrix with more rows than columns but will have a nonzero entry $\sigma_{i}$ in the $i^{th}$ row and column.
  ...
  \qitem We made the assumption that $A$ was a full rank matrix, but this will not always be the case. Therefore, in the case in which $A$ is of rank $k < n,$ we will use the \textbf{compact} SVD. This was derived in question 2 (g), but can be expressed in the following form:
  \begin{equation}
