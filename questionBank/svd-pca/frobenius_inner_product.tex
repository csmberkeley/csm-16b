...
\begin{enumerate}
  ...
  \sol {
    ...
    Since $U$ and $V^{T}$ are both square matrices with orthonormal columns,
    $\norm{U \Sigma V^{T}}_{F} = \norm{\Sigma V^{T}}_{F} = \norm{\Sigma}_{F}.$
  ...
  \sol {
    ...
    Since $\Sigma^{T} \Sigma$ is a diagonal matrix, that looks like:
    $$\begin{bmatrix}
    \sigma_{1}^{2} & \cdots & 0 \\
    \vdots & \ddots & \vdots \\
    0 & \cdots & \sigma_{n}^{2}
    \end{bmatrix}$$
    The trace of this matrix will be the sum of all of its diagonal entries and it'll follow that:
    $$\norm{A}_{F} = \sqrt{\sum\limits_{i = 1}^{n} \sigma_{i}^{2}}.$$
  }
