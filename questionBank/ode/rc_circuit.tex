...
\sol{
\begin{align*}
\intertext{We denote the voltage to the left of resistor to be $y$. We obtain the following differential equations. KCL at node between the resistor and capacitor gives}
...
\sol{
	\begin{align*}
		\intertext{In our case, we know}
		C\frac{dV_{\text{c}}}{dt} + \frac{V_{\text{c}}}{R} - \frac{V_{\text{S}}}{R}=0
		\intertext{which can be written as}
		\frac{dV_{\text{c}}}{dt} = - \frac{V_{\text{c}}}{RC} + \frac{V_{\text{S}}}{RC}
		\intertext{Therefore from the hint, we know that $\alpha = -\frac{1}{RC}$. We know that $V_c(0) = 0$ and $V_c = V_s$ from the previous parts.}
		\intertext{Therefore plugging in these values into hint 2, we get:}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=200]{\bank/ode/figures/tau_curve.jpg}
