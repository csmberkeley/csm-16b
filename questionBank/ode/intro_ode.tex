...
$y(t) = Ce^{-\alpha t}$ works for any nonzero choice of $C \in \mathbb{R}.$

}

\sol{
Notice that our solution isn't unique since $y(t) = Ce^{-\alpha t}$ 
satisfies our differential equation for any nonzero choice of $C.$
Plugging in $t = 0,$ we get $y(0) = C = 2,$ so our solution is
$y(t) = 2 e^{-\alpha t}.$
}
...
So by parts (b) and (c), our solution is $\widetilde{y}(t) = Ce^{-\alpha t}.$	
}
...
\sol{
Converting back to $y(t),$ our solution is $y(t) = Ce^{-\alpha t} + \beta / \alpha.$
Plugging in $t = 0,$ we get $y(0) = C + \beta / \alpha$ or $C = y(0) - \beta / \alpha.$
