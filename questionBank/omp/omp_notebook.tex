...
\begin{enumerate}
  ...
  \qitem Now that we've found the signal with highest correlation, what is the np.roll function doing?
  ...
  \sol {
    ...
    $\hat{y}_{1} = a_{2} \vec{s}_{2}^{\ (\tau_{2})}.$ We know what the signal $\vec{s}_{2}$ is and have shown that it is most likely that $\vec{y}$ contains $\vec{s}_{2}$ shifted by $7.$ Therefore, np.roll is delaying $\vec{s}_{2}$ by 7 time steps and we do this to compute $a_{2}$ using a Least-Squares estimate.
  }

  \qitem We subtract $\hat{y}_{1} = a_{2} \vec{s}_{2}$ from our original signal $\vec{y}$ to obtain the residual $\vec{r}.$ Can we say that our received signal $\vec{y}$ consists of only $\vec{s}_{2}?$
  ...
  \qitem We run cross correlation on all of the signals on more time. Why are we cross correlating $\vec{r}$ with all of the signals instead of $\vec{y}?$ Also,
  what do you notice about the cross correlations of $\vec{r}$ and $\vec{s}_{2}?$
