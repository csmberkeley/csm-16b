...
\begin{enumerate}
  ...
  We will call the matrix with orthonormal columns $U_{i}.$ What is the orthogonal projection matrix that takes a vector $\vec{y}$ in $\mathbb{R}^{n},$ and projects it onto the $\text{Col}(U_{i})?$

  \sol {
    The projection matrix that projected a vector $\vec{b}$ onto the $\text{Col}(A)$ in Least Squares, was shown to be $A (A^{T} A)^{-1} A^{T}.$ Since our matrix $U_{i}$ has orthonormal columns, we can say that the projection matrix onto $\text{Col}(U_{i})$ using Least Squares is $U_{i} (U_{i}^{T} U_{i})^{-1} U_{i}^{T}.$ We can show this is equal to:
    $$U_{i} (U_{i}^{T} U_{i})^{-1} U_{i}^{T} = U_{i} (I)^{-1} U_{i}^{T} = U_{i} U_{i}^{T}$$
  }

  \item Now using the projection matrix from the previous part, let's project the vector $\vec{y}$ onto the $\text{Col}(U_{i}).$
  What is the resulting projection?

  \sol {
    The resulting projection will be $U_{i} U_{i}^{T} \vec{y}.$ This can be simplified as follows:
    \begin{align*}
    \text{proj}_{\text{Col}(U_{i})} \vec{y} &= U_{i} U_{i}^{T} \vec{y} = U_{i} \\
    &= U_{i} \begin{bmatrix} 
    - & \vec{u}_{1}^{T} & - \\
    - & \vdots & - \\
    - & \vec{u}_{i}^{T} & - 
    ...
    | & | & | \\
    \vec{u}_{1} & \cdots & \vec{u}_{i} \\
    | & | & |
    \end{bmatrix}
    \begin{bmatrix} \vec{u}_{1}^{T} \vec{y} \\ \vdots \\ \vec{u}_{i}^{T} \vec{y} \end{bmatrix} \\
    &= (\vec{u}_{1}^{T} \vec{y}) \vec{u}_{1} + \cdots + (\vec{u}_{i}^{\ T} \vec{y}) \vec{u}_{i}
    \end{align*}
  }
  ...
  \sol {
    Our estimate at the $i^{\text{th}}$ iteration is $\hat{y}_{i} = (\vec{u}_{1}^{T} \vec{y}) \vec{u}_{1} + \cdots + (\vec{u}_{i}^{\ T} \vec{y}) \vec{u}_{i}$ \\
    On the $(i + 1)^{st}$ iteration, our estimate will be $\hat{y}_{i+1} = (\vec{u}_{1}^{T} \vec{y}) \vec{u}_{1} + \cdots + (\vec{u}_{i+1}^{\ T} \vec{y}) \vec{u}_{i+1}.$ \\
    The estimates between iteration $i$ and $i + 1$ only differ by $(\vec{u}_{i+1}^{\ T} \vec{y}) \vec{u}_{i+1}!$ 
    This means we no longer need to take inverses or use Least Squares to solve for our estimates $\hat{y}_{i}$ since we proved in the previous part that $\text{proj}_{\text{Col}(U_{i})} \vec{y}$ has a closed formula and can be updated every step by adding $(\vec{u}_{i+1}^{\ T} \vec{y}) \vec{u}_{i+1}.$ 
  }
  ...
  \sol {
    ...
    For our new algorithm, we will keep track of our orthonormal signals in a set $U$ initialized to be empty. In addition, we will initalize the residual vector $\vec{r} = \vec{y}.$ For the sake of notation, we will again index our signals from $1$ to $i - 1.$
