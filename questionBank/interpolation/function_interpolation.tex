...
\begin{enumerate}
	...
	\sol{ We have 4 data points given to use, each at the interval (or $\delta$) of $\frac{\pi}2$ then we can get our $\phi(x)$ for each point according to the equation 
			We have 4 data points given to use, each at the interval (or $\delta$) of $\frac{\pi}2$ then we can get our $\phi(x)$ for each point according to the equation
	\begin{equation*}
	\phi(x) = \begin{cases}  
	                1-\frac{1-\mid x \mid}{\frac{\pi}2} \hspace{1cm} \, |x|\leq\delta \\
	      			0 \hspace{1.6cm} \, \text{else} \\
	   		    \end{cases} 
	...
	\begin{equation*}
	\phi\(x) = \begin{cases}  
	                1-\frac{1-\mid x-\frac{\pi}{4} \mid}{\frac{\pi}2} \hspace{1cm} \, |x|\leq\delta \\
	      			0 \hspace{1.6cm} \, \text{else} \\
	   		    \end{cases} 
	...
	\begin{equation*}
	\phi(x) = \begin{cases}  
	                1-\frac{1-\mid x-\frac{3\pi}{4} \mid}{\frac{\pi}2} \hspace{1cm} \, |x|\leq\delta \\
	      			0 \hspace{1.6cm} \, \text{else} \\
	   		    \end{cases} 
	\end{equation*}
	\begin{equation*}
	\phi(x) = \begin{cases}  
	                1-\frac{\mid x- \frac{5\pi}{4} \mid}{\frac{\pi}2} \hspace{1cm} \, |x|\leq\delta \\
	      			0 \hspace{1.6cm} \, \text{else} \\
	   		    \end{cases} 
	\end{equation*}
	\begin{equation*}
	\phi(x) = \begin{cases}  
	                1-\frac-\mid x-\frac{7\pi}{4} \mid}{\frac{\pi}2} \hspace{1cm} \, |x|\leq\delta \\
	      			0 \hspace{1.6cm} \, \text{else} \\
	   		    \end{cases} 
	\end{equation*}
	We then scale each $\phi(x)$ by its respective y component and plot all of them on the graph. \\ 
		...
		\begin{center}
    		\includegraphics[width=4in]{\bank/interpolation/figures/linea_interpolation.png}
	  	\end{center}
		Next, we would add all of the graphs together so that it satisfies our equation where $y = \sum_i{y_i *\phi(x_i)}$ for each given point i. After scaling and adding all of our graphs together, here’s what it looks like:
		\begin{center}
    		\includegraphics[width=4in]{\bank/interpolation/figures/linea_interpolation_summed.png}
	  	\end{center}
	}
	...
	\sol{Once again, we separate each point by a phi function, which in this case is the sinc function: 
		\begin{equation*}
		\phi(x) = \begin{cases}  
	                \frac{sin(\pi * x)}{\ pi * x} \hspace{1cm} \, x\neq 0 \\
	      			1 \hspace{3cm} \, x=0 \\
	   		    \end{cases} 
		\end{equation*}
	...
	Is this similar to the last function? 
		...
		\sol{ There's a lot less symmetry in this function, and it also doesn't capture the same waveform: it's negative and positive in different areas. The function that we've sampled all of our points from is the sine function (how do parts a-c compare to the sine function?), but this last interpolation seems to be quite differnt .Is this from the asymettric sampling, or the varying frequency?
\begin{center}
