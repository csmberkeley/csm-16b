...
\begin{enumerate}
  ...
  \item The circuit can be simplified as:
    ...
    \[
      H(j \omega) = H_2(\omega)H_1(\omega)
    .\]


  \item Using the above transfer function, we have
    \[
      \abs{H(j \omega)} = \abs{H_2(\omega)H_1(\omega)}
      = \abs{H_2(\omega)}\abs{H_1(\omega)}
    .\]
    Since $H_1(\omega)$ is a transfer function of a low-pass filter and $H_2(\omega)$ is a transfer function of a high-pass filter, for values of $\omega$ approaches $0$, $\abs{H(j \omega)}$ approaches $0$.
