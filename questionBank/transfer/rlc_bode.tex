...
\meta{Poles are out of scope for 16B This semester. Even though we have 2nd order filters, Students only need to know how to make bode plots out of a transfer function. There is more nuance than this for 2nd or higher order filters, but it should be out of scope for this class.
...
To find the cutoff frequency, we need to set this into a form that we can set this into a form that we can easily solve for $H_C(j\omega)= \frac{1}{\sqrt{2}}$.

This is similar to having two first order (only two circuit elements) filters multiplied together, where the combined magnitude of the filter will have a form of $(\frac{1}{1 + \frac{j\omega}{\omega_c}})^2 = \frac{1}{(\frac{j\omega}{\omega_c})^2 + j\omega \frac{2E}{\omega_c} + 1}$ where $\omega_c$ is our cutoff frequency and $E$ is an expression involving $R$, $C$, and $L$ (that can be ignored). In this form, we can see that $\omega_c = \sqrt{1/LC} = 10^6$
