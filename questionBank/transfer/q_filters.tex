...
\begin{enumerate}
  ...
  \item The circuit can be simplified as:
    ...
    \[
      \widetilde{V}_{out} = \frac{Z_C}{Z_R + Z_C}V_{in} = H(\omega)\widetilde{V}_{in}
    .\]
...
\begin{enumerate}
  ...
  \item The transfer function $H(\omega)$.
    ...
    Using the voltage divider equation with impedance, we have
    \[
      V_{out} = \frac{Z_R}{Z_R + Z_C}V_{in} = H(\omega)V_{in}
    .\]
  ...
  \item Using the polar representation of complex numbers, we can rewrite the transfer function
    ...
    In this case, $V_{out} = 0$, it means that low frequencies are blocked by this filter.

  \item For values of $\omega$ approaches $\infty$, $\abs{H(\omega)}$ approaches $1$.
    In this case, $V_{out} = V_{in}$, it means that high frequencies can pass through this filter.
    \\
    \\
...
\begin{enumerate}
  ...
  \item Using the above transfer function, we have
    ...
    Therefore, $V_{out} = 0$, it means that low frequencies are blocked by this filter.
