...
\sol{
    ...
    \sol {
        ...
        The input to the system is 0 after the first time step, and the magnitude of $\vec{x}$ is conserved after every iteration, so
        \begin{align*}
            \lim_{t \to \infty} ||\vec{x}[t]|| = ||\vec{x}[1]|| = 16
        \end{align*}
