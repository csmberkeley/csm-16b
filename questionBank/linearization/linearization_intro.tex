...
\meta {
  We like to pick operating points $x^{*}$ in which $f(x^{*}) = 0$ since it will simplify $f(x) = f'(x^{*}) (x - x^{*}).$
}
...
\begin{enumerate}[resume]
  ...
  \qitem We will now look at a small window of $x(t)$ around $x^{*}$ and let $x(t) = x^{*} + \delta_{x}(t)$ where $\delta_{x}(t)$ is small enough so the error of the Taylor approximation is small. 
  Plug in this $x(t)$ into the original differential equation \eqref{eqn:nl} and use the first order Taylor approximation derived in the previous part to come up with a new differential equation in terms of $\delta_{x}(t).$
  ...
  \sol {
    ...
    Converting back to $x(t),$ we see that the new linearized approximation is $x(t) = x^{*} + \delta_{x}(t) = \delta_{x}(t) = \delta_{x}(0) e^{-t}.$
