...
\qitem Now, consider an arbitrary vector $\vec{v}_{2}$ in $S$. How can we create $\vec{u}_{2}$ for our orthonormal set? \\
...
\sol {
  ...
  $$\vec{q_2} = \vec{v_2} - (\vec{v}_{2}^T\vec{u_1})\vec{u_1}$$
...
\sol {
  ...
  We can repeat this for all vectors in $S$ to generate a new, orthonormal set $U$ that we can use in place of $S$ that is orthogonal and every vector has norm $1$ completing our algorithm.
}
\end{enumerate}