...
\begin{enumerate}
  ...
  \sol {
    ...
    $$\innp{\vec{x}}{\vec{y}} = 0.$$
    This means that $\norm{x} \norm{y} \cos(\theta) = 0.$ Since $\vec{x}, \vec{y} \neq \vec{0},$ $\norm{x}, \norm{y}$ must be non-zero, but this implies that $\cos(\theta) = 0.$
  ...
  \qitem Generalize the arugment above to show that if $\{\vec{v}_{1}, \cdots, \vec{v}_{n}\}$ is a set of non-zero orthogonal vectors, then the set must be linearly independent. We say a set of vectors is orthogonal, if each pair of vectors is mutually orthogonal, that is $\innp{\vec{v}_{i}}{\vec{v}_{j}} = 0$ for any $i$ and $j.$
